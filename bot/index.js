...
bot.callbackQuery("task_channel", async (ctx) => {
  ...
  const kb = new InlineKeyboard().url(t(user,"📢 Kanala Katıl","📢 Join Channel"), "https://t.me/HypeMiningCommunity").row().text(t(user,"✅ Katildim","✅ Joined"), "verify_channel");
...
bot.callbackQuery(/dep_ok_(.+)_(.+)/, async (ctx) => {
  ...
  // Referans %10 bonusu + yarisma puani
...
// Kanala yeni katilan uyeye hos geldin mesaji
...
// Admin: yarismayi bitir ve odulleri ver
...
// Madenci saatlik kazanim - her saat calisir
setInterval(() => {
  try {
    const db = loadDB();
...
// ADMIN: Ucretsiz gorev ekle
// Kullanim: /admintask  YouTube Kanali https://youtube.com/... 100